...
The purpose of this type is to add the information of the [`Range`](./Glsl-Range), i.e. where in the source code the
...
import Glsl.Range as Range exposing (Range)
...
{-| Combine three nodes, constructing a new node which will have the outer most range of the child nodes
-}
combine3 : (Node a -> Node b -> Node c -> d) -> Node a -> Node b -> Node c -> Node d
combine3 f ((Node { start } _) as a) b ((Node { end } _) as c) =
    Node
        { start = start, end = end }
        (f a b c)


{-| Combine four nodes, constructing a new node which will have the outer most range of the child nodes
-}
combine4 : (Node a -> Node b -> Node c -> Node d -> e) -> Node a -> Node b -> Node c -> Node d -> Node e
combine4 f ((Node { start } _) as a) b c ((Node { end } _) as d) =
    Node
        { start = start, end = end }
        (f a b c d)
