...
import Glsl exposing (BinaryOperation(..), Expression(..), RelationOperation(..), Stat(..), Statement(..), UnaryOperation(..))
...
innerValue : Context -> Expression -> Result Error ( Context, Value )
innerValue ctx e =
    case e of
        Float f ->
            Ok ( ctx, VFloat f )
...
autovectorizingFloatOp : Context -> String -> (Float -> Float) -> Expression -> Result Error ( Dict String Value, Value )
